...
export class YakaUI {
  ...
  initProgressBars() {
    document.querySelectorAll(".progress-fill").forEach((fill) => {
      ...
      ScrollTrigger.create({
        ...
        onLeaveBack: () => {
          gsap.to(fill, { width: 0, duration: 0.5, ease: "power2.in" });
        },
      });
    });
  }
  ...
  initImageReveal() {
    document.querySelectorAll(".reveal-image").forEach((reveal) => {
      ...
      tl.from(
        img,
        { xPercent: 101, scale: 1.3, duration: 1.2, ease: "power2.inOut" },
        "-=1.2",
      );
    });
  }
  ...
  initTypingText() {
    document.querySelectorAll("[data-typing]").forEach((el) => {
      ...
      ScrollTrigger.create({
        ...
        onLeaveBack: () => {
          gsap.to(el, { text: "", duration: 0.3, ease: "none" });
        },
      });
    });
  }
  ...
  initCircularProgress() {
    document.querySelectorAll(".progress-circle-wrapper").forEach((wrapper) => {
      ...
      ScrollTrigger.create({
        ...
        onEnter: () => {
          gsap.to(path, {
            strokeDashoffset: 283 - (val / 100) * 283,
            duration: 1.5,
            ease: "power2.out",
          });
          const obj = { val: 0 };
          gsap.to(obj, {
            ...
            duration: 1.5,
            onUpdate: () => {
              if (text) text.innerHTML = `${Math.floor(obj.val)}%`;
            },
          });
        },
        onLeaveBack: () => {
  ...
  initCursor() {
    ...
    document
      .querySelectorAll(
        "a, button, .tab-item, .tab-btn, .accordion-header, .interactive, .sidebar-link",
      )
      .forEach((el) => {
        el.addEventListener("mouseenter", () => {
...
const YakaComponents = (function () {
  ...
  const utils = {
    ...
    on(el, ev, h) {
      if (el) el.addEventListener(ev, h);
    },
    off(el, ev, h) {
      if (el) el.removeEventListener(ev, h);
    },
  };
  ...
  const Buttons = {
    init() {
      utils.$$(".glass-btn").forEach((btn) => {
        utils.on(btn, "click", () => {
          this.animate(btn, btn.getAttribute("data-anim"));
        });
      });
    },
    animate(btn, type) {
  ...
  const Checkbox = {
    init() {
      utils
        .$$(".cb-square input, .switch input, .dot-outer input")
        .forEach((cb) => {
          utils.on(cb, "change", () => {
  ...
  const Combobox = {
    ...
    init() {
      const input = utils.$("#comboInput"),
        button = utils.$("#comboButton"),
        options = utils.$("#comboOptions");
      if (!input || !button || !options) return;
      this.selected = this.data[1];
      this.render(options, input);
      utils.on(input, "focus", () => this.open(options));
      utils.on(input, "input", (e) =>
        this.filter(e.target.value, options, input),
      );
      utils.on(button, "click", () => this.toggle(options));
      utils.on(document, "click", (e) => {
        if (!e.target.closest("#combobox")) this.close(options, input);
      });
    },
    render(container, input, filteredData = this.data) {
      container.innerHTML = filteredData.length
        ? ""
        : '<div style="padding: 0.75rem; text-align: center; color: rgba(255,255,255,0.4);">No results found</div>';
    ...
    filter(q, c, i) {
      this.render(
        c,
        i,
        this.data.filter((p) => p.name.toLowerCase().includes(q.toLowerCase())),
      );
      this.open(c);
    },
    open(c) {
    ...
    close(c, i) {
      gsap.to(c, {
        ...
        onComplete: () => {
          ...
          if (this.selected) i.value = this.selected.name;
        },
      });
    },
    toggle(c) {
      c.style.display === "block" ? this.close(c) : this.open(c);
    },
  };
  ...
  const Listbox = {
    ...
    init() {
      const btn = utils.$("#listboxBtn"),
        options = utils.$("#listboxOptions"),
        label = utils.$("#selectedName");
  ...
  const RadioGroup = {
    ...
    render(container, isInitial = false) {
      ...
      this.plans.forEach((plan) => {
        ...
        opt.setAttribute("aria-checked", isChecked);
  ...
  const Transition = {
    ...
    init() {
      ...
      utils.$$(".dot").forEach((dot, i) => {
        utils.on(dot, "click", () => this.go(i));
      });
      if (utils.$("#prevBtn"))
        utils.on(utils.$("#prevBtn"), "click", () =>
          this.go((this.current - 1 + this.scenes.length) % this.scenes.length),
        );
      if (utils.$("#nextBtn"))
        utils.on(utils.$("#nextBtn"), "click", () =>
          this.go((this.current + 1) % this.scenes.length),
        );
      this.show(0);
    },
    show(i) {
    ...
    go(i) {
      this.apply(
        this.current,
        i,
        this.scenes[i].getAttribute("data-transition"),
      );
      this.show(i);
    },
    apply(from, to, type) {
      const f = this.scenes[from],
        t = this.scenes[to];
      switch (type) {
        case "elastic":
          gsap.to(f, {
          ...
          gsap.fromTo(
            t,
            { x: 100, opacity: 0 },
            { x: 0, opacity: 1, duration: 0.6, ease: "elastic.out(1, 0.8)" },
          );
          break;
        case "tilt":
          gsap.to(f, {
          ...
          gsap.fromTo(
            t,
            { rotationY: -35, opacity: 0 },
            { rotationY: 0, opacity: 1, duration: 0.7, ease: "power2.inOut" },
          );
          break;
        default:
          gsap.to(f, { opacity: 0, duration: 0.3 });
          gsap.fromTo(t, { opacity: 0 }, { opacity: 1, duration: 0.3 });
      }
    },
  };
  ...
  const Animations = {
    init() {
      window.addEventListener("load", () => {
        utils.$$(".glass-panel, .glass-card, .container-box").forEach((c) => {
          gsap.from(c, { opacity: 0, y: 20, duration: 1, ease: "power3.out" });
        });
      });
    },
  };
...
if (document.readyState === "loading")
  document.addEventListener("DOMContentLoaded", () => YakaComponents.init());
